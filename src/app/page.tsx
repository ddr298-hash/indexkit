...
const GUIDE_SECTIONS: GuideSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      "API 및 서비스 → 라이브러리에서 'Google Search Console API' 검색 후 활성화 (필수)",
      "(선택) 'Web Search Indexing API'도 활성화 — 아래 부록 기능용",
    ],
  },
  {
    title: "2단계 · 서비스 계정 발급",
  ...
  {
    title: "부록 · \"진단 / 색인 요청\" 버튼은 왜 있나요?",
    intro:
      "네이버 블로그 자체를 직접 인증해서 진단·색인 요청을 시도해볼 수 있는 기능도 남아있지만, " +
      "위에서 설명한 이유로 대부분 \"Permission denied\" 오류가 납니다. 혹시 몰라 남겨둔 보너스 " +
      "기능이니, 안 되더라도 허브 방식과는 무관하게 정상입니다.",
  },
];
    ...
    try {
      ...
      setAutoSetupResult(
        `GitHub 쪽 설정이 끝났습니다. 1~2분 후 ${hubDomain} 에서 허브가 열립니다. ` +
          `이제 Search Console에서 이 주소의 소유권만 확인하면 끝입니다 (앱 안내 참고).`,
      );
    } catch (err) {
      setGithubError(err instanceof Error ? err.message : String(err));
    } finally {
              ...
              {hasGithubConfig && (
                <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid color-mix(in srgb, var(--foreground) 12%, transparent)" }}>
                  <p className="hint" style={{ marginBottom: 8 }}>
                    ...
                    첫 배포까지 자동으로 실행합니다. (Search Console 소유권 확인만 별도로 필요 — 가이드 참고)
                  </p>
